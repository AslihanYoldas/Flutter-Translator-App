...
class Repository {
  ...
  Future <TranslatorResult?> fetchTranslateResult(TranslatorData data) async {
    ...
    TranslatorResult response = await _api.getTranslateResult(
        languages[data.sourceLan] ?? 'en', languages[data.targetLan] ?? 'tr',
        data.text ?? "");
    debugPrint("Repo = ${response.data?.translatedText}");
    return response;
    }
